...
const SA='fa75f8007dbf5e2197d46641dad2067676ec4bfb7e5e6f384a56e1ecfc8ebddc', SB='5e77d5e96f9ba34c1690e3d1857b6b4a814ff00ec24106c219d6092769877c48'
const UA='cmt8chgkn0001mbkaszteayf3', UB='cmt8chgkp0003mbka1kix5p44'
...
async function main(){
  ...
  // S2: like persistence
  const act=await api(SA,'POST','/api/social/activities',{verb:'ORDERED',objectType:'Restaurant',objectId:'reg-test',metadata:{restaurantName:'Reg Test'},visibility:'FRIENDS'})
  await wait(500)
  const like1=await api(SB,'POST',`/api/social/activities/${act.json?.activity?.id}/like`)
  const like2=await api(SB,'POST',`/api/social/activities/${act.json?.activity?.id}/like`) // dup
  T.push({gate:'S2_like_idempotent',result:like1.status===200&&like2.status===200&&like2.json?.likeCount===1?'PASS':'FAIL'})
  ...
  // S5D: activity realtime
  const s5dOutbox=await db.outbox.findFirst({where:{eventType:'SOCIAL_ACTIVITY_CREATED'}})
  T.push({gate:'S5D',result:s5dOutbox?.status==='PUBLISHED'?'PASS':'FAIL'})
