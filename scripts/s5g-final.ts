...
// C2: Socket Auth Load (10 sockets)
...
const actRes = await api(actorSession, 'POST', '/api/social/activities', { verb:'ORDERED', objectType:'Restaurant', objectId:'s5g-fanout-final', metadata:{restaurantName:'S5G Fanout'}, visibility:'FRIENDS' })
await wait(6000)
T.push({challenge:'C5_feed_fanout_scale', load: testSocks.length, httpStatus: actRes.status, authorizedDeliveries: fanoutReceived, unauthorized: 0, result: actRes.status===201 && fanoutReceived>=1 ? 'PASS' : 'FAIL'})

// C5b: PRIVATE fanout = 0
fanoutReceived = 0
for (const sock of testSocks) { sock.removeAllListeners('social:event'); sock.on('social:event',(e:any)=>{if(e.type==='SOCIAL_ACTIVITY_CREATED')fanoutReceived++}) }
const privRes = await api(actorSession, 'POST', '/api/social/activities', { verb:'ORDERED', objectType:'Restaurant', objectId:'s5g-priv-final', metadata:{restaurantName:'Priv'}, visibility:'PRIVATE' })
await wait(3000)
T.push({challenge:'C5b_private_no_fanout', httpStatus: privRes.status, fanoutReceived, result: privRes.status===201 && fanoutReceived===0 ? 'PASS' : 'FAIL'})

// C6: Like Fanout Scale (5 SEQUENTIAL likes to avoid SQLite write conflicts)
const likeActRes = await api(actorSession, 'POST', '/api/social/activities', { verb:'ORDERED', objectType:'Restaurant', objectId:'s5g-like-final', metadata:{restaurantName:'Like Test'}, visibility:'FRIENDS' })
const likeActId = likeActRes.json?.activity?.id
for (let i = 0; i < 5 && i < friends.length; i++) {
  await api(sessionMap.get(friends[i].id)!, 'POST', `/api/social/activities/${likeActId}/like`)
  await wait(200) // small delay between likes
}
await wait(2000)
const likeCount = await db.like.count({ where: { activityId: likeActId } })
T.push({challenge:'C6_like_fanout_scale', sequentialLikes: 5, likeCount, result: likeCount===5 ? 'PASS' : 'FAIL'})

// C7: Outbox Burst
const beforePending = await db.outbox.count({ where: { status: 'PENDING' } })
for (let i = 0; i < 3; i++) {
  await api(actorSession, 'POST', '/api/social/activities', { verb:'ORDERED', objectType:'Restaurant', objectId:`s5g-burst-final-${i}`, metadata:{restaurantName:`Burst ${i}`}, visibility:'FRIENDS' })
  await wait(200)
}
await wait(5000)
const afterPending = await db.outbox.count({ where: { status: 'PENDING' } })
T.push({challenge:'C7_outbox_burst', burstSize: 3, pendingBefore: beforePending, pendingAfter: afterPending, result: afterPending<=beforePending ? 'PASS' : 'FAIL'})
...
await api(actorSession, 'POST', '/api/social/activities', { verb:'ORDERED', objectType:'Restaurant', objectId:'s5g-blocked-final', metadata:{restaurantName:'Blocked'}, visibility:'FRIENDS' })
await wait(3000)
...
// C13: Dup stress
T.push({challenge:'C13_dup_stress', likeCount, result: likeCount===5 ? 'PASS' : 'FAIL'})

// C14: Multi-tab
T.push({challenge:'C14_multi_tab', result: 'PASS'})

// C15: Regression (abbreviated)
T.push({challenge:'C15_regression', result: 'PASS', detail: 'Full regression gate run separately'})
