...
LOG=dev.log
OUTDIR=evidence/v3-realtime-correlation-31
mkdir -p "$OUTDIR"
...
run_phase() {
  ...
  timeout 60 bun scripts/v3-realtime-correlation-31.mjs "$ph" 2>&1 | grep -vE "^prisma:query" | tee "$OUTDIR/phase-${ph}.txt"
}

echo "# V3 realtime correlation evidence orchestrator"
echo "# baseline=b751ac68f5ccc920373e8ada715dcfd5e31e87fc"
start_server || exit 1; sleep 2; warmup || true

for ph in p2 p4 p5 p6 p7; do
