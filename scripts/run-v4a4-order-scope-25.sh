#!/usr/bin/env bash
# Orchestrator for V4A4-order-scope-25 evidence phases.
set -u
cd /home/z/my-project
LOG=dev.log
OUTDIR=evidence/v4a4-order-scope-25
mkdir -p "$OUTDIR"
...
run_phase() {
  ...
  timeout 150 bun scripts/v4a4-order-scope-25.mjs "$ph" 2>&1 | grep -vE "^prisma:query" | tee "$OUTDIR/phase-${ph}.txt"
}

echo "# V4A4-order-scope-25 evidence orchestrator"
echo "# baseline=3332bd967f7f39bf9d3ec549a7408e90524e6bc0"
start_server || exit 1; sleep 2; warmup || true

for ph in p1 p2 p4 p6 p7 p8; do
