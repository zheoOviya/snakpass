...
console.log(`=== STEP 1: Connect ${N} sockets with reconnection:true ===`)
const connectPromises = tokens.map((u, i) => {
  ...
  return new Promise(resolve => {
    sock.on('connect', () => {
      socks[i] = sock
      perClient[i] = { userId: u.id, originalSocketId: sock.id, initialConnectedAt: Date.now(), disconnectAt: null, reconnectedSocketId: null, reconnectAt: null, reconciliationCompleteAt: null, authResult: null, manualReloadUsed: false }
      resolve(true)
    })
...
const initialConnected = initResults.filter(r => r).length
console.log(`Initial connected: ${initialConnected}/${N}`)

if (initialConnected < N) { console.log(`FAIL`); process.exit(1) }

// Set up listeners on each socket
socks.forEach((sock, i) => {
  sock.on('disconnect', () => { if (perClient[i] && perClient[i].disconnectAt === null) perClient[i].disconnectAt = Date.now() })
  sock.on('connect', () => {
    if (perClient[i] && perClient[i].reconnectAt === null && perClient[i].disconnectAt !== null) {
...
try { execSync('pkill -9 -f "bun.*realtime/index.ts" || true', { stdio: 'ignore' }) } catch {}
try { execSync('fuser -k 3003/tcp || true', { stdio: 'ignore' }) } catch {}
...
try {
  ...
  console.log(`Activity created during outage: ${actRes.status} (DB committed)`)
...
await new Promise(r => setTimeout(r, 5000))
console.log('Realtime should be up. Waiting for auto-reconnections...')
...
const sameSocketObjects = socks.filter((s, i) => s && perClient[i] && perClient[i].reconnectAt !== null).length
console.log(`Same socket objects reconnected: ${sameSocketObjects}/${N}`)
...
const testC = {
  ...
  details: { initialConnected, observedDisconnect, automaticReconnect: reconnectedCount, authFailures, manualReload, reconciliationCount, sameSocketObjects, serverSideMutationDuringOutage: 'Activity created during outage — DB committed via Next.js (port 3000, independent of realtime port 3003)' }
}
...
const matrix = {
  ...
  finalMatrix: [
    { scenario:'Socket connect', attempted:50, success:initialConnected, failed:0, unauthorized:0, p50:24, p95:27, max:31, result:'PASS', citation:'S5G-11 exact-50-closure.json' },
    ...
    { scenario:'Concurrent Like', attempted:5, success:5, failed:0, unauthorized:0, p50:'—', p95:'—', max:'—', result:'PASS', citation:'S5G-10 evidence' }
  ],
  closureRule: {
    socketConnect50: 'PASS (S5G-11)', reconnect50: testC.result, unauthorized: authFailures, manualReload: manualReload,
    validLatency: rInvariant === 'VALID', concurrentLike5: 'PASS',
    verdict: testC.result === 'PASS' ? 'S5G_VERIFIED' : 'S5G_BLOCKED'
  },
  perClientSample: perClient.slice(0, 10).map(c => ({ userId: c.userId.substring(0,12), originalSocketId: c.originalSocketId, disconnectAt: c.disconnectAt, reconnectedSocketId: c.reconnectedSocketId, reconnectAt: c.reconnectAt, reconnectLatencyMs: c.reconnectAt ? c.reconnectAt - restartTime : null, authResult: c.authResult, manualReloadUsed: c.manualReloadUsed }))
}
