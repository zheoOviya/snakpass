...
cp prisma/schema.prisma /tmp/schema-4c.prisma.bak
sed -i 's/provider = "postgresql"/provider = "sqlite"/' prisma/schema.prisma
cp .env /tmp/env-4c.bak
...
EVIDENCE_TEST_MODE=true bun run dev > /tmp/dev-4c.log 2>&1 &
DEV_PID=$!
...
for i in $(seq 1 45); do
  ...
  if ! kill -0 $DEV_PID 2>/dev/null; then echo "[wrapper] Server died"; tail -30 /tmp/dev-4c.log; cp /tmp/schema-4c.prisma.bak prisma/schema.prisma; sed -i 's/provider = "sqlite"/provider = "postgresql"/' prisma/schema.prisma; cp /tmp/env-4c.bak .env; echo 'DATABASE_URL=file:/home/z/my-project/db/custom.db' > .env; bunx prisma generate > /dev/null 2>&1; exit 1; fi
  sleep 1
done

if [ "$READY" = "0" ]; then echo "[wrapper] Failed"; tail -30 /tmp/dev-4c.log; cp /tmp/schema-4c.prisma.bak prisma/schema.prisma; sed -i 's/provider = "sqlite"/provider = "postgresql"/' prisma/schema.prisma; cp /tmp/env-4c.bak .env; echo 'DATABASE_URL=file:/home/z/my-project/db/custom.db' > .env; bunx prisma generate > /dev/null 2>&1; exit 1; fi

echo "[wrapper] Running 4c evidence script..."
EVIDENCE_BASE_URL=http://127.0.0.1:3000 node scripts/wave4-4c-evidence.mjs
EVIDENCE_EXIT=$?
...
cp /tmp/schema-4c.prisma.bak prisma/schema.prisma
sed -i 's/provider = "sqlite"/provider = "postgresql"/' prisma/schema.prisma
cp /tmp/env-4c.bak .env
