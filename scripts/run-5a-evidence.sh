...
cp prisma/schema.prisma /tmp/schema-5a.prisma.bak
sed -i 's/provider = "postgresql"/provider = "sqlite"/' prisma/schema.prisma
cp .env /tmp/env-5a.bak
...
EVIDENCE_TEST_MODE=true bun run dev > /tmp/dev-5a.log 2>&1 &
DEV_PID=$!
...
for i in $(seq 1 45); do
  ...
  if ! kill -0 $DEV_PID 2>/dev/null; then echo "[wrapper] Server died"; tail -30 /tmp/dev-5a.log; cp /tmp/schema-5a.prisma.bak prisma/schema.prisma; sed -i 's/provider = "sqlite"/provider = "postgresql"/' prisma/schema.prisma; cp /tmp/env-5a.bak .env; echo 'DATABASE_URL=file:/home/z/my-project/db/custom.db' > .env; bunx prisma generate > /dev/null 2>&1; exit 1; fi
  sleep 1
done

if [ "$READY" = "0" ]; then echo "[wrapper] Failed"; tail -30 /tmp/dev-5a.log; cp /tmp/schema-5a.prisma.bak prisma/schema.prisma; sed -i 's/provider = "sqlite"/provider = "postgresql"/' prisma/schema.prisma; cp /tmp/env-5a.bak .env; echo 'DATABASE_URL=file:/home/z/my-project/db/custom.db' > .env; bunx prisma generate > /dev/null 2>&1; exit 1; fi

echo "[wrapper] Running 5a E1-E5 evidence script..."
EVIDENCE_BASE_URL=http://127.0.0.1:3000 EVIDENCE_DB=sqlite node scripts/wave5-5a-evidence.mjs
EVIDENCE_EXIT=$?
...
cp /tmp/schema-5a.prisma.bak prisma/schema.prisma
sed -i 's/provider = "sqlite"/provider = "postgresql"/' prisma/schema.prisma
cp /tmp/env-5a.bak .env
